  ...
  <ul class="list bg-neutral text-neutral-content rounded-box shadow-md max-w-screen-lg mt-5 mx-4 lg:mx-auto">
    ...
    <dialog id="image_modal" class="modal">
      <div class="modal-box relative p-1 rounded-none">
        ...
        <form method="dialog">
          <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 text-2xl bg-re">✕</button>
        </form>

        <img
          v-if="selectedImage"
          :src="selectedImage"
          class="w-auto h-auto" alt="imagen ampliada" />
      </div>
